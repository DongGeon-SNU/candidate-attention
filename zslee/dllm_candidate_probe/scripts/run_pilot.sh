#!/usr/bin/env bash
# At most ten prompts. The Python entrypoint refuses pilots estimated above 30 minutes.
set -euo pipefail
...
LOG_FILE="${PROBE_ROOT}/logs/pilot_${STAMP}.log"
...
"${PYTHON_BIN}" "${PROBE_ROOT}/scripts/run_counterfactual_probe.py" \
  ...
  --mode pilot \
  --probe-root "${PROBE_ROOT}"
echo "pilot completed; log: ${LOG_FILE}"
