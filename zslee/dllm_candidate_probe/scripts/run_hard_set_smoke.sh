#!/usr/bin/env bash
# Validate pilot reuse and exact size-5 LOO branches before the larger audit.
set -euo pipefail
...
LOG_FILE="${PROBE_ROOT}/logs/hard_set_smoke_${STAMP}.log"
...
"${PYTHON_BIN}" "${PROBE_ROOT}/scripts/preflight.py" --probe-root "${PROBE_ROOT}" --environment-json "${PROBE_ROOT}/outputs/environment_report.json" --stage smoke
"${PYTHON_BIN}" "${PROBE_ROOT}/scripts/run_hard_set_audit.py" --config "${PROBE_ROOT}/configs/hard_set_audit.yaml" --mode smoke --probe-root "${PROBE_ROOT}"
echo "hard-set smoke completed; log: ${LOG_FILE}"
